...
(async () => {
  ...
  /* ====== PHASE 26 — Gmail import ====== */
  {
    ...
    check('an unsupported attachment is marked as such',
      /not a PDF or photo/.test(openTxt), openTxt.slice(0, 400));
    await page.click('.ematt:has-text("terms.docx")');
    await page.waitForTimeout(700);
    check('and tapping it explains rather than failing',
      /only pdfs and photos/i.test(await page.textContent('#toast').catch(() => '')),
      await page.textContent('#toast').catch(() => 'no toast'));
    await page.screenshot({ path: `${ROOT}/n38-email.png` });

    /* ---------- importing into the existing receipt form ---------- */
    await page.click('.ematt:has-text("invoice-9001.pdf")');
    await page.waitForTimeout(3000);
    ...
    await page.waitForTimeout(1400);
    await page.click('.ematt:has-text("statement.png")');
    ...
    await page.waitForTimeout(1400);
    await page.click('.ematt:has-text("credit-note.pdf")');
    await page.waitForTimeout(3000);
